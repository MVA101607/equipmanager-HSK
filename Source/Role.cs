...
namespace EquipmentManager
{
    internal class Role : IExposable
    {
        ...
        private const string DefaultRolesPackageId = "LordKuper.EquipmentManager";
        private const string DefaultRolesRelativePath = "Common/default_roles.xml";

        public static readonly IEnumerable<Role> DefaultRoles = LoadDefaultRoles();

        private static IEnumerable<Role> LoadDefaultRoles()
        {
            try
            {
                var path = ResolveDefaultRolesPath();
                if (path == null || !File.Exists(path))
                {
                    Log.Warning(
                        $"Equipment Manager: default roles file not found at '{path ?? DefaultRolesRelativePath}', using empty defaults.");
                    return Array.Empty<Role>();
                }
                var roles = ReadDefaultRolesFromXml(path);
                ApplyTranslatedLabels(roles);
                return roles;
            }
            catch (Exception ex)
            {
                Log.Error($"Equipment Manager: failed to load default roles: {ex.Message}");
                return Array.Empty<Role>();
            }
        }

        private static string ResolveDefaultRolesPath()
        {
            var mod = LoadedModManager.RunningModsListForReading?.FirstOrDefault(m =>
                DefaultRolesPackageId.Equals(m?.PackageId, StringComparison.OrdinalIgnoreCase));
            if (mod == null) { return null; }
            return Path.Combine(mod.RootDir, DefaultRolesRelativePath.Replace('/', Path.DirectorySeparatorChar));
        }

        private static void ApplyTranslatedLabels(IList<Role> roles)
        {
        ...
        private static List<Role> ReadDefaultRolesFromXml(string path)
        {
            ...
            {
                Log.Error($"Equipment Manager: 'Loadouts' node not found in {path}");
                return roles;
            }
            using var loadouts = xmlReader.ReadSubtree();
            if (!loadouts.ReadToDescendant("li")) { return roles; }
            do
            {
                using var roleSubtree = loadouts.ReadSubtree();
                var role = ReadRole(roleSubtree);
                if (role != null) { roles.Add(role); }
            } while (loadouts.ReadToNextSibling("li"));
            return roles;
        }

        private static Role ReadRole(XmlReader xmlReader)
        {
            if (!xmlReader.ReadToFollowing("li") || !xmlReader.Read()) { return null; }
            var id = 0;
            var label = string.Empty;
            var priority = 0f;
            var primaryRuleType = PrimaryWeaponType.None;
            ...
            while (true)
            {
                ...
                switch (xmlReader.Name)
                {
                    ...
                    case "Label":
                        ...
                        break;
                    case "Priority":
                        priority = xmlReader.ReadElementContentAsFloat();
                        break;
                    ...
                    case "SkillWeights":
                        ...
                        {
                            _ = xmlReader.Read();
                            skillWeights.Add(ReadSkillWeight(xmlReader));
                    ...
                    case "StatWeights":
                        ...
                        {
                            _ = xmlReader.Read();
                            statWeights.Add(ReadStatWeight(xmlReader));
                            xmlReader.ReadEndElement();
                        }
                        xmlReader.ReadEndElement();
                        break;
                    default:
                        if (!xmlReader.Read()) { return BuildRole(); }
                        break;
                }
            }
            return BuildRole();
            ...
            Role BuildRole()
            {
                var role = new Role(id)
                {
                    Label = label,
                    Priority = priority,
                    PrimaryRuleType = primaryRuleType,
                ...
                role.RangedSidearmRules.AddRange(rangedSidearmRules);
                role.MeleeSidearmRules.AddRange(meleeSidearmRules);
                foreach (var kv in pawnTraits) { role.PawnTraits[kv.Key] = kv.Value; }
                foreach (var kv in pawnWorkCapacities) { role.PawnWorkCapacities[kv.Key] = kv.Value; }
                role.SkillWeights.AddRange(skillWeights);
                role.StatWeights.AddRange(statWeights);
                return role;
            }
        }
